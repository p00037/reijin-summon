...
test("移動先が左ならスプライトを反転し、右なら反転しない", () => {
  assert.equal(spriteFlipXForMovement("Player", { x: 0, y: 0 }, { x: 1, y: 0 }), false);
  assert.equal(spriteFlipXForMovement("Player", { x: 0, y: 0 }, { x: -1, y: 0 }), true);
  assert.equal(spriteFlipXForMovement("Cpu", { x: 2, y: 0 }, { x: 3, y: 0 }), false);
  assert.equal(spriteFlipXForMovement("Cpu", { x: 2, y: 0 }, { x: 1, y: 0 }), true);
});

test("停止中は従来通りチームの向きに戻す", () => {
  assert.equal(spriteFlipXForMovement("Player", { x: 0, y: 0 }, { x: 0.001, y: 0 }), false);
  assert.equal(spriteFlipXForMovement("Cpu", { x: 0, y: 0 }, { x: 0.001, y: 0 }), true);
});
