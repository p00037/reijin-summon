...
test("保存がない場合は標準デッキを選択した初期ライブラリを返す", () => {
  ...
  assert.deepEqual(result.library, {
    ...
    decks: [{ id: "standard", name: "標準デッキ", cardIds: ["SC020", "SC019", "SC014"] }]
  });
});
...
test("複数デッキを保存して順序ごと復元する", () => {
  ...
  const library: DeckLibrary = {
    ...
    decks: [
      { id: "first", name: "低コスト", cardIds: ["SC002", "SC004"] },
      { id: "second", name: "標準デッキ", cardIds: ["SC020", "SC019", "SC014"] }
    ]
  };
...
test("保存先が例外を投げてもクラッシュせず失敗を報告する", () => {
  const result = saveDeckLibrary({ setItem() { throw new DOMException("quota", "QuotaExceededError"); } }, {
    version: 1, selectedDeckId: null, decks: []
  });
...
test("不正なライブラリは保存先を上書きせず失敗を報告する", () => {
  let writes = 0;
  const invalid = { version: 1, selectedDeckId: null, decks: [{ id: "x", name: "x", cardIds: "SC001" }] };
  ...
  const invalidLibraries = [
    {
      version: 1,
      selectedDeckId: null,
      decks: [{ id: "", name: "名前なし", cardIds: ["SC001"] }]
    },
    {
      version: 1,
      selectedDeckId: null,
      decks: [{ id: "   ", name: "空白ID", cardIds: ["SC001"] }]
    },
    {
      version: 1,
      selectedDeckId: "same",
      decks: [
        { id: "same", name: "一つ目", cardIds: ["SC001"] },
        { id: "same", name: "二つ目", cardIds: ["SC002"] }
      ]
    }
  ];
