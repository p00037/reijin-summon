...
import { createDefaultBattleState, findLeader, findUnit, isUnitAlive } from "../core/battleState.js";
import type { BattleCommand, BattleConfig, BattleState, MatchResult, TeamId, UnitId } from "../core/types.js";
import { tickAbilities, tryUseAbility } from "./abilitySystem.js";
import { countCompletedElementals, removeDestroyedElementals, tickElementalBuilds, tryBeginElementalBuild } from "./elementalSystem.js";
import { recordLeaderDamageForMp, tickMpRecovery, tryReviveUnit } from "./resurrectionSystem.js";
import { canSummon, tickSummonGauges, tickSummonedUnits, tryExecuteSummon } from "./summonSystem.js";
import { applyMoveCommand, calculateUnitHealingElapsed, markDefeatedUnits, tickCombat, tickMovement, tickUnitHealing } from "./unitSystem.js";
import { tryPlaceInitialUnit } from "./initialPlacement.js";
...
export class GameSession {
  ...
  applyCommand(command: BattleCommand): void {
    ...
    if ('unitId' in command) {
      const unit = this.state.units.find(candidate => candidate.unitId === command.unitId);
      if (!unit || unit.team !== command.team) return;
    }
    if ('targetPosition' in command && (!Number.isFinite(command.targetPosition.x) || !Number.isFinite(command.targetPosition.y))) return;
