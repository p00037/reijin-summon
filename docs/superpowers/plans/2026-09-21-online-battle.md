...
タスク1〜7の実装を追加し、既存CPU戦を含む356件の試験で確認。タスク8のローカル2画面・10人負荷確認を実施。タスク9は公開設定例と手順まで用意した。実機の複数指操作、全操作の目視確認、公開アカウント・別ネットワークでの確認は残っているため、全計画の完了とはしない。詳細は[検証記録](../../online-battle-validation.md)を参照。以下は当初の手順チェックリストとして保持し、実施結果は検証記録を正とする。
